...
function scroll(id) {
  document.getElementById(id.split(" ")[0].toLowerCase()).scrollIntoView();
  scrollNav(id);
}
...
function scrollNav(id) {
  ...
  if (document.querySelector(`[title='${id}']`)) {
    document.querySelector(`[title='${id}']`).classList.add("nav-selected");
  }
}
